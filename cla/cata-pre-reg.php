...
/*Preparacion de variables*/
    ...
    if(!$eCodPresentacion)
        {
            $insert = " INSERT INTO CatPresentaciones
            (
            tNombre
			)
            VALUES
        ...
        else
        {
            $insert = "UPDATE 
                        ...
                        SET
                            tNombre= $tNombre
                            WHERE
                            eCodPresentacion = ".$eCodPresentacion;
        ...
        $eCodPresentacion = $eCodPresentacion ? $eCodPresentacion : mysql_insert_id();
...
if(!sizeof($errores))
{
    $tDescripcion = "Se ha ".(($bTipo==1) ? 'insertado' : 'actualizado')." el producto ".sprintf("%07d",$eCodPresentacion);
