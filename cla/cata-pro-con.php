...
$bOrden = $data->rOrden;
$rdOrden = $data->rdOrden ? $data->rdOrden : 'eCodProducto';

switch($accion)
{
    case 'D':
                $insert = "UPDATE CatProductos SET eCodEstatus=7 WHERE eCodProducto = ".$codigo;
        break;
    case 'F':
    ...
    case 'C':
        ...
        WHERE 1=1".
        ($eCodProducto ? " AND cc.eCodProducto = ".$eCodProducto : "").
        ...
        while($rConsulta=mysql_fetch_array($rsConsulta)){
            //imprimimos
       $tHTML .=    '<tr>'.
        '<td>'.menuEmergenteJSON($rConsulta{'eCodProducto'},'cata-pro-con').'</td>'.
...
{      
    ...
    if(!sizeof($errores))
    {
        $tDescripcion = "Se ha ".(($accion=="D") ? 'Eliminado' : 'Finalizado')." el producto código ".sprintf("%07d",$codigo);
